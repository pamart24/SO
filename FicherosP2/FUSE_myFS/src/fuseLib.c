...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi)
{
	...
	while (totalRead < bytes2Read){
		...
		//offset(pos actual) / BLOCK_SIZE_BYTES(tamaño del bloque en bites) = numero de bloque en el que estoy
		...
		// buffer <- currentBlock del disco vistual
		if(readBlock(&myFileSystem, currentBlock,&buffer)!= 0)
			return -EIO;
...
{
	int indexNode,indexDir;
	//NodeStruct* node;

	//Buscamos el path del archivo en el sistema de ficheros
	if((indexDir = findFileByName(&myFileSystem,(char *) path+1) == -1)){
		return -ENOENT;// si no lo encontramos, terminamos
	}
	//cogemos el indice del nodo recorriendo las variables de los structs del myFS.h
	indexNode = myFileSystem.directory.files[indexDir].nodeIdx;
	//cogemos el nodo del struct
	//node = myFileSystem.nodes[indexNode];

	//truncar el fichero(lo dejamos a 0 para borrarlo)
	if(resizeNode(indexNode,0)!= 0){
		return -EIO;
	}

	//marcar la entrada del directorio como libre
	myFileSystem.directory.files[indexDir].freeFile = true;
